        ...
        **kargs
        ...
        if self.multi_target_cond:
            if self.multi_encoder_type == "multi":
                self.embed_target_cond = EmbedTargetLocMulti(
                    self.all_goal_joint_names, self.latent_dim
                )
            elif self.multi_encoder_type == "single":
                self.embed_target_cond = EmbedTargetLocSingle(
                    self.all_goal_joint_names, self.latent_dim, self.target_enc_layers
                )
            elif self.multi_encoder_type == "split":
                self.embed_target_cond = EmbedTargetLocSplit(
                    self.all_goal_joint_names, self.latent_dim, self.target_enc_layers
                )
    ...
    def forward(self, x, timesteps, y=None):
        ...
        if "target_cond" in y.keys():
            ...
            time_emb += self.mask_cond(
                self.embed_target_cond(
                    y["target_cond"], y["target_joint_names"], y["is_heading"]
                )[None],
                force_mask=y.get("target_uncond", False),
            )  # For uncond support and CFG
