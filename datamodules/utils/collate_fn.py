...
def collate(batch):
    ...
    if "orig_lengths" in notnone_batches[0]:
        cond["y"].update(
            {
                "orig_lengths": torch.as_tensor(
                    [b["orig_lengths"] for b in notnone_batches]
                )
            }
        )
        ...
        cond["y"].update(
            {
                "start": torch.as_tensor(
                    [b["start"] for b in notnone_batches], dtype=torch.float32
                )
            }
        )
        ...
        cond["y"].update(
            {
                "goal": torch.as_tensor(
                    [b["goal"] for b in notnone_batches], dtype=torch.float32
                )
            }
        )
